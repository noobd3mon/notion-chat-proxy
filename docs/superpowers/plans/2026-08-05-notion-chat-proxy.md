...
## Addendum: Model picker + per-request model (added after Tasks 1–8)

**Feature (from the user's pasted `getAvailableModels` capture + fetch example):** expose the Notion model picker list and let `/api/chat` choose a model per request. Implemented + tested (66 tests green).

**Endpoints / contract:**
- `GET /api/models` (Bearer `API_KEY`, 401 without) → worker calls `POST /api/v3/getAvailableModels` with body `{ spaceId }` + headers `x-notion-active-user-header` + `x-notion-space-id` (both already set by `notionHeaders` when `userId`/`spaceId` are passed) → returns `{ "models": [...] }` transformed to a clean picker shape: `id` (codename), `name`, `family`, `provider`, `displayGroup`, `disabled`, `disabledReason`, `supportedReasoningEfforts`, `defaultReasoningEffort`. Internal-only fields (`modelCardAttributes`, `workflow`/`customAgent`/`agentService` beta flags, `restrictedAccessModelsInPickerConfig`) are dropped. On Notion non-2xx → `502 { error }`.
- `POST /api/chat` now accepts optional `body.model` (a codename from `/api/models`). Defaults to `NOTION_MODEL` (`fireworks-kimi-k3`). When set to something other than the default, the Worker validates it against the available list and rejects disabled/unknown models with `400 { error: "invalid model: ..." }` **before** opening the SSE stream. When `body.model` equals the default, validation is skipped (the common path adds no Notion/KV work). If the list can't be loaded (fetch fail), validation **fails open** — the requested model is passed through and Notion rejects it if invalid.

**Cache:** `src/models.js` holds a module-level in-memory cache (`{ data, fetchedAt }`, TTL ~1h) so `/api/chat` validation does not add a Notion call or a KV read on the warm path. Soft/isolate-scoped — different isolates may fetch independently; acceptable since the list changes rarely and validation is best-effort. `_resetCache()`/`_isCached()` are exported test hooks.

**Refactor in `worker.js`:** `activeSpace` is now resolved once in the `fetch` handler (1 KV read — same as before, just moved up from `runTurn`) and passed into both the model-validation path and `streamChat`/`runTurn`. `runTurn` now takes `{ env, kv, activeSpace, messages, message, model, write }` and reassigns `activeSpace` only on rotation.
